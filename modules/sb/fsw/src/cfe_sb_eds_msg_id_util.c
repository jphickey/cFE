...
size_t CFE_SB_MsgHdrSize(const CFE_MSG_Message_t *MsgPtr)
{
    ...
    if (HasSecHdr)
    {
        ...
        if (MsgType == CFE_MSG_Type_Cmd)
        {
            ...
            InterfaceId = EDS_INTERFACE_ID(CFE_SB_Telecommand);
        ...
        else if (MsgType == CFE_MSG_Type_Tlm)
        {
            ...
            InterfaceId = EDS_INTERFACE_ID(CFE_SB_Telemetry);
    ...
    if (TopicId != 0)
    {
        EdsStatus = CFE_MissionLib_GetArgumentType(SBINTF_DB, InterfaceId, TopicId, 1, 1, &EdsId);
    }
