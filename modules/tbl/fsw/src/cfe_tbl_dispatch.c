...
void CFE_TBL_TaskPipe(CFE_SB_Buffer_t *SBBufPtr)
{
    ...
    CmdStatus = CFE_TBL_Application_Component_Telecommand_Dispatch(CFE_SB_Telecommand_indication_Command_ID, SBBufPtr,
    ...
    if (CmdStatus < CFE_SUCCESS)
    {
        ...
        if (CmdStatus == CFE_TBL_NOT_IMPLEMENTED || CmdStatus == CFE_STATUS_BAD_COMMAND_CODE)
        {
    ...
    else if (CmdStatus < 0 && CmdStatus != CFE_STATUS_UNKNOWN_MSG_ID)
    {
        /*
